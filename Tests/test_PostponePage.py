...
""" to run all tests command: pytest Tests/test_PostponePage.py
    to run all tests and log to html format: pytest Tests/test_PostponePage.py -v --html=./hubSpot.html 
    to run all tests and log to html format in parallel mode execution: 
    pytest Tests/test_PostponePage.py -v -n 3 --html=./hubSpot.html """
...
class TestPostponeAtHomePage(BaseTest):
    ...
    def test_quantity_of_postponed_books_at_the_header(self):
        ...
        self.list_of_books = self.postponePage.find_several_element(PostponePage.HEART_SYMBOL_AT_HOME_PAGE)
        self.first_book = self.list_of_books[0]
        self.postponePage.scroll_to_element(self.first_book)
    ...
    """test that name of the book marked as postponed at the home page is equal 
    book name at the postpone page"""
    ...
    def test_deletion_of_all_books_postponed_at_postpone_page(self):
        ...
        alert.accept()
        assert self.postponePage.get_element_text(PostponePage.POSTPONED_BOOKS_DELETED_MESSAGE) == TestData. \
            SUCCESSFUL_DELETION
    ...
    """ test that postponed book from Postpone page "Отложено" can be moved into Basket "Корзина" by 
    button "В КОРЗИНУ" which is placed below postponed book at the Postpone page and status of button 
    "В КОРЗИНУ" changed to "ОФОРМИТЬ" """
    ...
    def test_that_button_move_into_basket_changed_to_checkout(self):
        ...
        # read the id of book before click
