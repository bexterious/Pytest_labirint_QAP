...
class TestHomePageMainMenu(BaseTest):
    ...
    and leads to proper page - submenu "Билингвы" """
    ...
    def test_to_set_current_region_entered_cyrillic_by_latin_letters_works_via_auto_advice(self):
        ...
        self.auto_advice_cities = self.homePage.find_several_element(HomePage.REGION_GUESS_LUST)
        self.auto_advice_cities[0].click()
        assert self.homePage.get_element_text(HomePage.REGION_CURRENT_SETTING) == TestData. \
            FIRST_CITY_IN_AUTO_ADVICE_IN_CYRILLIC

    """ test that popup window appear when move over "Сообщение" button 
    (keep in mind that "is_visible" is bool method) """
