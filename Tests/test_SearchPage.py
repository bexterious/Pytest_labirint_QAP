...
""" to run all tests command: pytest Tests/test_SearchPage.py
    to run all tests and log to html format: pytest Tests/test_SearchPage.py -v --html=./hubSpot.html 
    to run all tests and log to html format in parallel mode execution: 
    pytest Tests/test_SearchPage.py -v -n 3 --html=./hubSpot.html """
...
class TestSearchPage(BaseTest):
    ...
    # this test check that Button "ВСЕ ФИЛЬТРЫ" displayed at the page body open hiding Submenu "ВСЕ ФИЛЬТРЫ"
    ...
    def test_that_submenu_search_opens_and_disabling_digital_books_works(self):
        ...
        # check that there are no books with label "Электронные книги" at the page
        for book in self.list_of_digital_books:
            assert self.searchPage.element_is_not_visible(book)

    # this test check that Button "Бумажные книги" displayed at the page body hiding Submenu "ВСЕ ФИЛЬТРЫ" clickable
